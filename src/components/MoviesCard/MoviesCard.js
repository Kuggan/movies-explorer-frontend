...
function MoviesCard(props) {
    ...
    return (
        <li className="movies__list-item">
            ...
            <a href={props.saved ? props.movie.trailer : props.movie.trailerLink}  target="_blank" rel="noreferrer" className="movies__trailer-link">
                <img alt={props.movie.nameRU} src={props.saved ? props.movie.image : image} className="movies__list-poster"/>
            </a>
            {props.saved ?
                    <button className={`movies__list-delete-button ${isDeleteButtonVisible ? 'movies__list-delete-button_visible' : ''}`}  onClick={handleDeleteMovie}></button> :
