        ...
        <section className="login">
            ...
            <form className="login__form auth__form" onSubmit={handleLogin}> 
                <fieldset className="login__fields auth__fields">
                    ...
                    <input type="email" name="email" className="login__input auth__input"   autoComplete="on"
                    ...
                    <input type="password" name="password" className="login__input auth__input"   autoComplete="on"
                     value={values.password || ''} onChange={handleChange}
                     required minLength="8" disabled={props.isSaving}/>
                    <span className="login__error auth__error">{errors.password}</span>
                </fieldset>
                <button type="submit" disabled={!isFormValid}
                 className={`login__submit-button auth__submit-button ${isFormValid ? '' : 'auth__submit-button_disabled'}`}>Войти</button>
            </form>
